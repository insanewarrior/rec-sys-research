...
Val: the operation acts at the value-mixing stage of attention.
\begin{equation}
\begin{aligned}
...
intensity-driven direction change in the output vector, the
...
Variant & Mechanism \\
\midrule
IA-SASRec-Add & $\mathrm{softmax}\!\left(\dfrac{QK^{\top}}{\sqrt{d}} + \lambda\, M_W + M\right)\! V$ \\[1.2em]
IA-SASRec-Mul & $\mathrm{softmax}\!\left(\dfrac{QK^{\top}}{\sqrt{d}} \odot \bigl(1 + \lambda(M_W - 1)\bigr) + M\right)\! V$ \\[1.2em]
IA-SASRec-Val & $\left[\,\mathrm{softmax}\!\left(\dfrac{QK^{\top}}{\sqrt{d}} + M\right) \odot \bigl(1 + \lambda(M_W - 1)\bigr)\right] V$ \\[0.6em]
\bottomrule
\end{tabular}
\end{table*}
